		...
		for (int y = 0; y < Size; y++){
			for (int z = 0; z < Size; z++){
				if (Blocks[GetBlockIndex(x, y, z)] != EBlock::Air){
					...
					for (auto Direction : {
						     EDirection::Front, EDirection::Right, EDirection::Back, EDirection::Left, EDirection::Up,
						     EDirection::Down
					     }){
...
bool AChunk::Check(const FVector Position) const{
	if (Position.X >= Size || Position.Y >= Size || Position.X < 0 || Position.Y < 0 || Position.Z < 0){
		return true;
	}
	if (Position.Z >= Size){
		return true;
...
FVector AChunk::GetPositionInDirection(const EDirection Direction, const FVector Position) const{
	switch (Direction){
	case EDirection::Front: return Position + FVector::ForwardVector;
...
FVector AChunk::GetNormal(const EDirection Direction) const{
	switch (Direction){
	case EDirection::Front: return FVector::ForwardVector;
